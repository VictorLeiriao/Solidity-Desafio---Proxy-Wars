...
error AccountBlocked(address user); // <-- NOVO ERRO AQUI!
...
contract BankV7 is Initializable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    // V7 (FICHA SUJA / BANIDOS) <-- ADICIONADO AQUI PARA NÃO QUEBRAR O STORAGE
    ...
    function registerRequest(string memory _name, uint256 _age, string memory _country) public {
        // MÁGICA AQUI 👇 Impede que um usuário banido tente se registrar de novo!
    ...
    function blockAccount(address _account) public onlyOwner {
        isWhitelisted[_account] = false; // Tira os direitos
        isBlocked[_account] = true;      // Marca a ficha suja
    }

    function approveAccount(address _account) public onlyOwner {
        isWhitelisted[_account] = true;  // Dá os direitos
        isBlocked[_account] = false;     // Limpa a ficha suja
    }
